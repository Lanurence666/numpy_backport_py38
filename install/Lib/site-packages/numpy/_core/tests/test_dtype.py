...
class TestDTypeClasses:
    ...
    def test_basic_dtypes_subclass_properties(self, dtype):
        ...
        if dtype.type.__name__ != "rational":
            dt_name = (type(dtype).__name__.lower()[:-len("dtype")] if "dtype" and type(dtype).__name__.lower().endswith("dtype") else type(dtype).__name__.lower())
    ...
    def test_integer_alias_names(self, int_, size):
        ...
        assert (DType.__name__.lower()[:-len("dtype")] if "dtype" and DType.__name__.lower().endswith("dtype") else DType.__name__.lower()) == sctype.__name__
...
class TestDTypeSignatures:
    ...
    def test_signature_dtypes_classes(self, typename: str):
        ...
        _typename = typename.lower()[:-len("dtype")] if "dtype" and typename.lower().endswith("dtype") else typename.lower()
        if _typename in ("bytes", "str"):
            params_expect = {"size"}
        elif _typename == "void":
            params_expect = {"length"}
        elif _typename in ("datetime64", "timedelta64"):
            params_expect = {"unit"}
        elif _typename == "string":
