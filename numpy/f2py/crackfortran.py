...
declaration: type = <typespec> [[<attrspec>]::] <entitydecl>
<typespec> = byte                          |
...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        last_name = None
        for l in ll:
            l[0], l[1] = (l[0].strip()[len(','):] if l[0].strip().startswith(',') else l[0].strip()), l[1].strip()
            if l[0].startswith('('):
...
def get_parameters(vars, global_params={}):
    ...
    for n in param_names:
        if '=' in vars[n]:
            ...
            dimspec = ([s[len('dimension'):] if s.startswith('dimension') else s.strip()
