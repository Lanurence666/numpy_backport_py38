...
def find_existing_pythoncapi_compat(root):
    existing = []
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames[:] = [d for d in dirnames if d not in SKIP_DIRS]
...
def update_existing_pythoncapi_compat_h(root, project_config=None):
    updated = []
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames[:] = [d for d in dirnames if d not in SKIP_DIRS]
...
def deploy_pythoncapi_compat_h(root, files_needing_compat, project_config=None):
    if project_config:
    ...
    existing_locations = find_existing_pythoncapi_compat(root)
...
def main():
    ...
    updated_compat = update_existing_pythoncapi_compat_h(root, project_config)
    if updated_compat:
    ...
    deployed = deploy_pythoncapi_compat_h(root, files_needing_compat, project_config)
    if deployed:
    ...
    print()
    print("[7/7] 最终验证...")
    c_files_after = find_c_files(root)
